...
public class ConsultaControllerTest {
    ...
    @Test
    public void deveCancelarUmaConsultaCadastrada() throws Exception {
        var dadosCancelamentoConsulta = new DadosCancelamentoConsulta(MotivoCancelamento.MEDICO_CANCELOU, null);

        when(consultaService.cancelarConsulta(anyLong(), any(DadosCancelamentoConsulta.class))).thenReturn(true);

        mockMvc.perform(delete("/consulta/1")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(dadosCancelamentoConsulta)))
            .andExpect(status().isNoContent());
    }

    @Test
    public void deveRetornar500_casoHajaErroAoPersistirOCancelamento() throws Exception {
        var dadosCancelamentoConsulta = new DadosCancelamentoConsulta(MotivoCancelamento.MEDICO_CANCELOU, null);
        when(consultaService.cancelarConsulta(anyLong(), any(DadosCancelamentoConsulta.class))).thenReturn(false);

        mockMvc.perform(delete("/consulta/1")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(dadosCancelamentoConsulta)))
            .andExpect(status().isInternalServerError())
            .andExpect(jsonPath("$.message").value("Erro ao efetuar o cancelamento da consulta, tente novamente em instantes"));
    }

    @Test
    public void deveRetornar400_casoNaoTenhaConsultaComIdInformado() throws Exception {
        var dadosCancelamentoConsulta = new DadosCancelamentoConsulta(MotivoCancelamento.MEDICO_CANCELOU, null);
        when(consultaService.cancelarConsulta(anyLong(), any(DadosCancelamentoConsulta.class))).thenThrow(new IllegalArgumentException("Nenhuma consulta agendada com o ID informado"));

        mockMvc.perform(delete("/consulta/2")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(dadosCancelamentoConsulta)))
            .andExpect(status().isBadRequest())
            .andExpect(jsonPath("$.message").value("Nenhuma consulta agendada com o ID informado"));
    ...
    @Test
    public void deveRetornar400_casoTenteCancelarComMenosDe24hParaAConsulta() throws Exception {
        var dadosCancelamentoConsulta = new DadosCancelamentoConsulta(MotivoCancelamento.PACIENTE_DESISTIU, null);

        when(consultaService.cancelarConsulta(anyLong(), any(DadosCancelamentoConsulta.class))).thenThrow(new IllegalArgumentException("Você só pode cancelar uma consulta com 24h de antecedência"));

        mockMvc.perform(delete("/consulta/1")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(dadosCancelamentoConsulta)))
            .andExpect(status().isBadRequest())
            .andExpect(jsonPath("$.message").value("Você só pode cancelar uma consulta com 24h de antecedência"));
    }
}
